...
//  M3 Admin
//
//  Created by Happy Sanz Tech on 12/12/19.
...
class Alert: NSObject {
    ...
    public class var defaultManager : Alert {
           struct Static {
       ...
       public func showErrorAlert(error: Error) {
           if error._code == NSURLErrorNotConnectedToInternet {
               ...
               self.showOkAlert("Error!", message: "Device is offline. Please check the network connection and try again.", handler: nil)
           }
       }

       //Displays Logout Alert Message
       public func showLogoutAlert() {
           Alert.defaultManager.showOkAlert("User Logout!", message: "Someone has used your credentials to login into HealthePets. Please click on OK to login again.") { (response) in
